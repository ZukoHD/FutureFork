...
getgenv()._Future_ForkVERSION = "1.1.6 | "..(shared.Future_ForkDeveloper and "dev" or shared.Future_ForkTester and  "test" or "release").." build" -- // This is a cool thing yes
getgenv()._Future_ForkMOTD = ".gg/EgHeBptA4q 🔥"
print("[Future_Fork] Loading!")
repeat wait() until game:IsLoaded()
if shared.Future_Fork~=nil then print("[Future_Fork] Detected Future_Fork already executed, not executing!") return end
getgenv().Future_ForkStartTime = game:GetService("Workspace"):GetServerTimeNow()
...
local function requesturl(url, bypass) 
	...
	local repourl = bypass and "https://raw.githubusercontent.com/ZukoHD/" or "https://raw.githubusercontent.com/ZukoHD/FutureFork/main/"
	local url = url:gsub("Future_Fork/", "")
...
local GuiLibrary = loadstring(requesturl("Future_Fork/GuiLibrary.lua"))()
shared.Future_Fork.GuiLibrary = GuiLibrary
...
local function getscript(id) 
	local id = id or shared.Future_ForkPlaceId or game.PlaceId
	id = tostring(id)
	local req = requesturl("Future_Fork/scripts/"..id..".lua")
	if type(req) == "string" then
		return loadstring(req)()
	else
		--fwarn("[Future_Fork] invalid script (error "..tostring(req)..")") -- game is not supported
	end
end

local function getplusscript(id) -- Future_Fork plus moment
	local id = id or shared.Future_ForkPlaceId or game.PlaceId
	id = tostring(id)
	local req = requesturl("Future_Fork/plus/"..id..".fp")
	if type(req) == "string" then
		return loadstring(req)()
	else
		--fwarn("[Future_Fork] invalid script (error "..tostring(req)..")") -- game is not supported
...
local configBox; configBox = configButton.CreateTextbox({
	...
	["Function"] = function(value)
		spawn(function()
			...
			if isfile("Future_Fork/configs/"..tostring((shared.Future_Fork and shared.Future_Fork.PlaceId) or game.PlaceId).."/"..value..".json") then
...
if table.find(bedwarsidtable, game.PlaceId) then 
	shared.Future_ForkPlaceId = 6872274481
end
...
if table.find(minerscaveidtable, game.PlaceId) then 
	shared.Future_ForkPlaceId = 6604417568
end
...
getplusscript()
if success then 
	print("[Future_Fork] Successfully retrieved Universal script!")
else
	fwarn("Unsuccessful attempt at retrieving Universal script!\n report this in the discord.\n (".._error..")")
end
if success2 then 
	print("[Future_Fork] Successfully retrieved Game script!")
else
	fwarn("Unsuccessful attempt at retrieving Game script!\n report this in the discord.\n (".._error2..")")
end
--[[
if success3 then 
    print("[Future_Fork] Successfully retrieved Future_ForkPlus Game script!")
else
    fwarn("Unsuccessful attempt at retrieving Future_ForkPlus Game script!\n report this in the discord.\n (".._error3..")")
end]]
...
spawn(function()
	repeat
		if not shared.Future_Fork then 
			break
		end
